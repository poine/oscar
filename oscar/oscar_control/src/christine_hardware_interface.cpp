...
#define __NAME "christine_hardware_interface"
  ...
  gear_enc_res_(GEARBOX*ENCODER_RES)
{
  ...
  for (int i=0; i<NB_JOINTS; i++) {
    joint_position_[i] = 0.; joint_velocity_[i] = 0.; joint_effort_[i] = 0.;
    joint_effort_command_[i] = 0.; joint_position_command_[i] = 0.;
...
bool OscarHardwareInterface::start() {
  ...
  rc_servo_send_pulse_normalized(MOTOR_SERVO_CH, 0.);
...
void OscarHardwareInterface::write() {
  ...
  float motor = 0.2*joint_effort_command_[0];
  if (motor>1.) motor=1.; if (motor<-1.) motor=-1.;
  //struct timeval tv;
  //gettimeofday(&tv, NULL );
  //double now = tv.tv_sec + (0.000001f * tv.tv_usec);
  //double now =ros::Time::now().toSec();
  //float motor = 0.1*sin(now);
  rc_servo_send_pulse_normalized(MOTOR_SERVO_CH, motor);
  //ROS_INFO(" write HW %f %f", motor, steering);
}
...
int main(int argc, char** argv)
{
  ...
  ROS_INFO_STREAM_NAMED(__NAME, "Christine hardware node starting...");
